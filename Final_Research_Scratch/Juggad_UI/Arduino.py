        ...
        while True:
            ...
            if arduino_socket_q:
                ...
                if "Scanning for devices..." in line:
                    ...
                    print(f"Arduino_script: bike details: {bike_details}")
...
def display_bike_options(bike_details, root, callback_select_bike, callback_back_to_scan_screen):
    ...
    print("Arduino_script: Trying to display bike options")
    for widget in root.winfo_children():
        widget.destroy()

    # Show bike options as buttons
    for idx, bike in enumerate(bike_details):
        btn = tk.Button(root, text=f"{bike}", width=50,  font=("Helvetica", 40),command=lambda b=bike: callback_select_bike(b))
        btn.pack(pady=20)
